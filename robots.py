...
def render():
    ...
    st.markdown("""
    <div style="background: url('https://www.bing.com/images/search?view=detailV2&ccid=BGBXN%2Fl5&id=A170719060BF13998E674E0B42E827077D1EAD46&thid=OIF.PlZjVEwU2R35KSrAseMUSQ&mediaurl=https%3A%2F%2Fwww.shutterstock.com%2Fimage-photo%2Frobot-hand-ai-artificial-intelligence-260nw-2118695231.jpg&cdnurl=https%3A%2F%2Fth.bing.com%2Fth%2Fid%2FR.04605737f9793eb4acd0248c816190aa%3Frik%3D%26pid%3DImgRaw%26r%3D0&exph=280&expw=515&q=robot+background&simid=7191484086628&FORM=IRPRST&ck=3E5663544C14D91DF9292AC0B1E31449&selectedIndex=70&itb=0&cw=691&ch=640&ajaxhist=0&ajaxserp=0'); background-size: cover; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
        <h1 class="robots-header">ROBOTS Decisions</h1>
        <h3 style="color: white;">Logistic Regression Classification</h3>
    </div>
    """, unsafe_allow_html=True)
    
    # Display Spider-Man quote
    show_themed_quote("robots", "general")
    ...
    if 'Target' not in df.columns:
        st.warning("No binary target column found. robot will create one for you!")
    ...
    if missing_values > 0:
        ...
        missing_strategy = st.radio(
            "robots's Missing Value Strategy:",
    ...
    if selected_categorical_features:
        st.info("Robot  will automatically convert categorical features using Label Encoding.")
    ...
    # Train the model
    if st.button("Analyze with robo-algorithm", key="robots_analyze"):
        with st.spinner("robot is analyzing the data with his Spider-Sense..."):
            ...
            # Show success quote
            show_themed_quote("robots", "success")
    ...
    if st.session_state.model_results is not None:
        ...
        <div class="results-container">
            <h2 class="robots-header">Logistic Regression Results</h2>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            st.markdown(f"""
            <div class="robots-metric-container">
                <div class="robots-metric-label">Accuracy</div>
                <div class="robots-metric-value">{model_results['accuracy_test']:.4f}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            st.markdown(f"""
            <div class="robots-metric-container">
                <div class="robots-metric-label">Precision</div>
                <div class="robots-metric-value">{model_results['precision']:.4f}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col3:
            st.markdown(f"""
            <div class="robots-metric-container">
                <div class="robots-metric-label">Recall</div>
                <div class="robots-metric-value">{model_results['recall']:.4f}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col4:
            st.markdown(f"""
            <div class="robots-metric-container">
                <div class="robots-metric-label">F1 Score</div>
                <div class="robots-metric-value">{model_results['f1']:.4f}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        fig_conf = px.imshow(
            conf_matrix_df,
            text_auto=True,
            color_continuous_scale=['#8A2BE2', '#0d47a1'],
            aspect="equal",
            title="ROBOT's Confusion Matrix"
        )
        ...
        fig_roc = px.area(
            x=fpr, y=tpr,
            title=f'ROBOT\'s ROC Curve (AUC = {roc_auc:.4f})',
        ...
        <div class="robots-panel">
            ...
            <p>"robot-Sense is tingling! Let's predict something!"</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Make prediction
        if st.button("Make Prediction", key="robots_predict"):
            # Create input array
            ...
            st.markdown(f"""
            <div class="robots-panel">
                <h3>ROBOT's Prediction</h3>
                <p>The predicted class is:</p>
                <h2 style="color: {'#8A2bE2' if prediction_class == 1 else '#0d47a1'}; text-align: center;">
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                domain={'x': [0, 1], 'y': [0, 1]},
                title={'text': "ROBOT Probability"},
                gauge={
                    'axis': {'range': [0, 1]},
                    'bar': {'color': "#8A2be2"},
                    'steps': [
                        {'range': [0, 0.5], 'color': "#0d47a1"},
                        {'range': [0.5, 1], 'color': "#8a2be2"}
                    ],
                    'threshold': {
        ...
        create_download_buttons(df, model_results['model_summary'], None, "robots")