...
def render():
    ...
    st.markdown("""
    <div style="background: url('https://www.bing.com/images/search?view=detailV2&ccid=%2BCPdrC0b&id=92EB1CF68EDCEB7751AA2845F00FF22C46F5ED7D&thid=OIP.-CPdrC0bJeHvLkN7j7jcMAHaEK&mediaurl=https%3A%2F%2Fth.bing.com%2Fth%2Fid%2FR.f823ddac2d1b25e1ef2e437b8fb8dc30%3Frik%3Dfe31RizyD%252fBFKA%26riu%3Dhttp%253a%252f%252fs1.picswalls.com%252fwallpapers%252f2014%252f01%252f29%252fdragon-wide-wallpaper_063726704_21.jpg%26ehk%3D7QfXRVpXigbW%252fs2zH%252fFhl2qE85FFFjlQomfFeNpb6eg%253d%26risl%3D%26pid%3DImgRaw%26r%3D0&exph=1080&expw=1920&q=dragon++background&simid=608030592674714468&FORM=IRPRST&ck=A889F296413D4DEB48F21FE6165E5942&selectedIndex=2&itb=0&cw=691&ch=640&ajaxhist=0&ajaxserp=0'); background-size: cover; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
        <h1 class="dragons-header">💎 DRAGONS' Infinity Clusters</h1>
        <h3 style="color: white;">K-Means Clustering Analysis</h3>
    </div>
    """, unsafe_allow_html=True)
    
    # Display Thanos quote
    show_themed_quote("dragons", "general")
    ...
    n_clusters = st.slider("Select Number of Clusters (dragon eggs):", 2, 8, 4)
    ...
    if st.button("Snap! Find the Clusters", key="dragons_analyze"):
        with st.spinner("dragon is balancing the universe..."):
            ...
            # Show success quote
            show_themed_quote("dragons", "success")
    ...
    if st.session_state.model_results:
        ...
        <div class="results-container">
            <h2 class="dragons-header">K-Means Clustering Results</h2>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            st.markdown(f"""
            <div class="dragons-metric-container">
                <div class="dragons-metric-label">Number of Clusters</div>
                <div class="dragons-metric-value">{model_results['model_summary']['Number of Clusters']}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            st.markdown(f"""
            <div class="dragons-metric-container">
                <div class="dragons-metric-label">Silhouette Score</div>
                <div class="dragons-metric-value">{model_results['silhouette']:.4f}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col3:
            st.markdown(f"""
            <div class="dragons-metric-container">
                <div class="dragons-metric-label">Inertia</div>
                <div class="dragons-metric-value">{model_results['inertia']:.2f}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if len(selected_features) >= 2:
            ...
            fig_scatter = px.scatter(
                ...
                y=y_feature,
                color='Cluster',
                title=f"DRAGONS' Infinity Clusters ({x_feature} vs {y_feature})",
                color_discrete_sequence=px.colors.qualitative.Bold
            )
        ...
        if len(selected_features) >= 3:
            ...
            fig_radar.update_layout(
                ...
                title="dragons' eggs Power Profiles",
                plot_bgcolor='rgba(0,0,0,0.1)',
                paper_bgcolor='rgba(0,0,0,0.1)',
                font=dict(color='white')
            )
        ...
        <div class="dragons-panel">
            ...
            <p>" Your data will be classified."</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Make prediction
        if st.button("Classify New Data", key="dragons_predict"):
            # Create input array
            ...
            st.markdown(f"""
            <div class="dragons-panel" style="text-align: center;">
        ...
        create_download_buttons(model_results['df_with_clusters'], model_results['model_summary'], None, "dragons")