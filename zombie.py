...
def render():
    ...
    st.markdown("""
    <div style="background: url('https://www.bing.com/images/search?view=detailV2&ccid=ldXcsL0m&id=DB3B6F92216B8B0BD328DF9291A1EAA80F9D57D4&thid=OIP.ldXcsL0mMg3EzPrNcEU8VAHaEJ&mediaurl=https%3A%2F%2Fpng.pngtree.com%2Fthumb_back%2Ffw800%2Fbackground%2F20230722%2Fpngtree-horde-of-terrifying-zombies-swarm-evoking-halloween-horror-3d-render-image_3795773.jpg&cdnurl=https%3A%2F%2Fth.bing.com%2Fth%2Fid%2FR.95d5dcb0bd26320dc4ccfacd70453c54%3Frik%3D1FedD6jqoZGS3w%26pid%3DImgRaw%26r%3D0&exph=538&expw=960&q=zombie+background+&simid=608013812254770664&FORM=IRPRST&ck=7F95198C1F288E85983E0072879B7CD9&selectedIndex=13&itb=0&cw=691&ch=640&ajaxhist=0&ajaxserp=0'); background-size: cover; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
        <h1 class="batman-header">ZOMBIE Lab</h1>
        <h3 style="color: #e6e6e6;">Linear Regression Analysis</h3>
    </div>
    """, unsafe_allow_html=True)
    
    
    show_themed_quote("zombie", "general")
    ...
    if missing_values > 0:
        ...
        missing_strategy = st.radio(
            "zombies Missing Value Strategy:",
    ...
    # Train the model
    if st.button("Analyze with zombi's Precision", key="batman_analyze"):
        with st.spinner("zombie is analyzing the data..."):
            ...
            # Show success quote
            show_themed_quote("zombie", "success")
    ...
    if st.session_state.model_results:
        ...
        <div class="results-container">
            <h2 class="robots-header">Linear Regression Results</h2>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            st.markdown(f"""
            <div class="zombie-metric-container">
                <div class="zombie-metric-label">R-squared (Test)</div>
                <div class="zombie-metric-value">{model_results['test_r2']:.4f}</div>
            </div>
            """, unsafe_allow_html=True)
        with col2:
            st.markdown(f"""
            <div class="zombie-metric-container">
                <div class="zombie-metric-label">MSE (Test)</div>
                <div class="zombie-metric-value">{model_results['test_mse']:.4f}</div>
            </div>
            """, unsafe_allow_html=True)
        with col3:
            st.markdown(f"""
            <div class="zombie-metric-container">
                <div class="zombie-metric-label">RMSE (Test)</div>
                <div class="zombie-metric-value">{np.sqrt(model_results['test_mse']):.4f}</div>
            </div>
            """, unsafe_allow_html=True)
        with col4:
            st.markdown(f"""
            <div class="zombie-metric-container">
                <div class="zombie-metric-label">Intercept</div>
                <div class="zombie-metric-value">{model_results['model'].intercept_:.4f}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        fig_importance = px.bar(
            ...
            orientation='h',
            color='Coefficient',
            color_continuous_scale=['#008000', '#1a1'],
        ...
        # Make prediction
        if st.button("Make Prediction", key="zombie_predict"):
            # Create input array
        ...
        create_download_buttons(df, model_results['model_summary'], None, "zombie")